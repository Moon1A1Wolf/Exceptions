...
public class Program
{
    ...
    {
        Random rand = new Random(DateTime.Now.Second);
        Assert.Throws<Exception>(() => { Student student = new Student(Globals.names[rand.Next(Globals.names.Length)], Globals.surnames[rand.Next(Globals.surnames.Length)], rand.Next(-18, -1), $"{Globals.streets[rand.Next(Globals.streets.Length)]}, {rand.Next(1, 99)}", $"+380{rand.Next(111111111, 999999999)}", rand.Next(0, 100)); });
        Assert.Throws<Exception>(() => { Student student = new Student(Globals.names[rand.Next(Globals.names.Length)], Globals.surnames[rand.Next(Globals.surnames.Length)], rand.Next(18, 23), $"{Globals.streets[rand.Next(Globals.streets.Length)]}, {rand.Next(1, 99)}", $"+380{rand.Next(111111111, 999999999)}", rand.Next(-100, -1)); });
        Assert.Throws<Exception>(() => { Student student = new Student(Globals.names[rand.Next(Globals.names.Length)], Globals.surnames[rand.Next(Globals.surnames.Length)], rand.Next(18, 23), $"{Globals.streets[rand.Next(Globals.streets.Length)]}, {rand.Next(1, 99)}", $"+380{rand.Next(111111111, 999999999)}", rand.Next(101, 200)); });

        Assert.Throws<Exception>(() => { Group group = new Group(new List<Student>(), "", "Specialization", 1); });
        Assert.Throws<Exception>(() => { Group group = new Group(new List<Student>(), "Group 2", "", 1); });
        Assert.Throws<Exception>(() => { Group group = new Group(new List<Student>(), "Group 3", "Specialization", -1); });
